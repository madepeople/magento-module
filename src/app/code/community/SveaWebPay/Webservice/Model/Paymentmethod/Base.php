...
class SveaWebPay_Webservice_Model_Paymentmethod_Base extends Mage_Payment_Model_Method_Abstract
{
    ...
    protected function updateShippingAndBilling($order,$validCustomer = null)
    {
        ...
        $firstname     = (isset($validCustomer->LegalName)) ? $validCustomer->LegalName : "";
        $addressline1  = (isset($validCustomer->AddressLine1)) ? $validCustomer->AddressLine1 : "";
        $addressline2  = (isset($validCustomer->AddressLine2)) ? $validCustomer->AddressLine2 : "";
        $postarea      = (isset($validCustomer->Postarea)) ? $validCustomer->Postarea : "";
        $postcode      = (isset($validCustomer->Postcode)) ?  $validCustomer->Postcode : "";

        $phone = (isset($validCustomer->PhoneNumber) && $validCustomer->PhoneNumber != "") ? $validCustomer->PhoneNumber : null;
        $billingPhone  = ($phone === null) ? $session->getBillingPhone() : $phone;
        $shippingPhone = ($phone === null) ? $session->getShippingPhone() : $phone;
    ...
    */
    ...
    {    
        $swpSession = Mage::getSingleton("sveawebpayws/session");
        $swpSession->setOrderPlaceRedirectUrl( Mage::getUrl( "checkout/onepage/success", array('_secure'=>true) ) );
    ...
    public function retrieveOrderInformation($order)
    {
        ...
        $paymentArray = Mage::app()->getRequest()->getParam("payment");
        if(is_null($paymentArray) || !is_array($paymentArray))
            return;
        
        if(!key_exists($this->_code."_security_number",$paymentArray))
            return;
                    
        // Post Variables
        $securityNumber = $paymentArray[$this->_code."_security_number"];
        $email          = (key_exists($this->_code."_email",$paymentArray)) ? $paymentArray[$this->_code."_email"] : "";
        $isComp         = (key_exists($this->_code."_iscmp",$paymentArray)) ? $paymentArray[$this->_code."_iscmp"] : "";
        ...
        $phone = (key_exists($this->_code."_phone",$paymentArray)) ? $paymentArray[$this->_code."_phone"] : "";
